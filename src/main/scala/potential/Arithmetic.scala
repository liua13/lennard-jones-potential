// Copied from https://github.com/ucb-bar/gemmini/blob/master/src/main/scala/gemmini/Arithmetic.scala
...
case class DivisionResult(expWidth: Int, sigWidth: Int) extends Bundle {
  val bits = Float(expWidth, sigWidth)
  val valid = Bool()
  val ready = Bool()
  val flags = Bits(5.W)
}
  ...
  implicit object SIntArithmetic extends Arithmetic[SInt] {
    override implicit def cast(self: SInt) = new ArithmeticOps(self) {
      ...
      override def divider(denom_t: UInt): Option[(DecoupledIO[UInt], DecoupledIO[SInt])] = {
        ...
        // Instantiate the hardfloat divider
  ...
  implicit object FloatArithmetic extends Arithmetic[Float] {
    ...
    override implicit def cast(self: Float): ArithmeticOps[Float] = new ArithmeticOps(self) {
      ...
      override def /(denom: Float): Option[DivisionResult] = {
        ...
        divider.io.a := self_rec
        divider.io.b := denom_rec_resized

        val result = Wire(DivisionResult(self.expWidth, self.sigWidth))
        result.bits.bits := fNFromRecFN(self.expWidth, self.sigWidth, divider.io.out)
        result.valid := divider.io.outValid_div
        result.ready := divider.io.inReady
        result.flags := divider.io.exceptionFlags
